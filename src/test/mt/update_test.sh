...
it_parses_the_latest_multitool_location () {
  testing_url="http://files.cascading.org/multitool/2.2/multitool-latest.tgz"
...
it_complains_if_curl_fails_to_fetch_latest () {
  ...
  module_exit () {
    [ "$*" = "Cannot get latest multitool from http://files.cascading.org/multitool/2.2/latest.txt" ] && tested=1
  }
  mt_update_create_temp () {
    [ "$tested" = "1" ] && tested=2
  }
...
it_allows_a_version_specifier () {
  ...
  test "$mt_update_latest" = "http://files.cascading.org/multitool/2.2/multitool-latest.tgz"

  route_perform update --version more_latest
  test "$mt_update_latest" = "http://files.cascading.org/multitool/2.2/multitool-more_latest.tgz"

  route_perform update --version=super_latest
  test "$mt_update_latest" = "http://files.cascading.org/multitool/2.2/multitool-super_latest.tgz"
}
